...
1. **Secrets niemals committen.** Alle Zugangsdaten in `.env.local` (lokal) bzw. Vercel-Env-Variablen. `.env*` gehört in `.gitignore`. Kein Passwort, Token oder Key im Code oder in Commits.
...
🔴 **HUMAN CHECKPOINT A:** Bitte den Menschen, `.env.local` mit Kickbase-Zugangsdaten, `KICKBASE_LEAGUE_IDS` und Supabase-URL/Service-Key zu füllen, und ein Supabase-Projekt anzulegen. Warte auf Bestätigung.
...
🔴 **HUMAN CHECKPOINT B:** Bitte den Menschen, `scripts/capture-fixtures.sh` mit gültigem Token laufen zu lassen und die JSON unter `/fixtures/` zu committen. Danach baust du alle Parser/Tests gegen diese Fixtures.
